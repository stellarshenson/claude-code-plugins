...
- **svg-infographics** - the umbrella skill is named `svg-infographics`, after its plugin, the way the `datascience`, `journal`, `project-management` and `autobuild` umbrella skills are; `svg-designer` is gone. The fork call is `Skill(skill="svg-infographics:svg-infographics")`, and every route into it names the new skill - the plugin's `create`, `fix`, `validate` and `beautify` skills and `theme` and `fix` commands, the devils-advocate `run` and `evaluate` skills and popular-science adversary, the datascience popular-science skill and command, the rule-bundle path in `svg_tools/manifest.py`, the example banners, the tests and the cassette notes. A caller writing `svg-infographics:svg-infographics` by analogy with the other four plugins had been getting `Unknown skill`
...
- **devils-advocate** - `review-tools dossier` found no subcommands on Python 3.12: the help probe parsed argparse's invalid-choice error for quoted names, and 3.12.13 prints the list bare (`choose from run, fly`) where 3.11 and 3.13 quote it. The probe now reads both shapes; a test feeds it each one. CI's 3.12 matrix entry had been red since v1.7.15 on the two dossier tests
- **repository** - the root README's showcase link points at `docs/showcase/`, where the v1.7.15 cleanup moved the directory; the link-checker job had failed on it since
- **project-management** - `report` no longer mixes status with level on one SUMMARY row (DEF-PMGT-51). The grid is two grids, one per axis: status - `| Category | Open | Fixed | Rejected | Total |` (`Done` on criteria, `Worked on` after Open when an item is locked) - then the open items by level under `Open by severity` or `Open by importance`, ending in the Open count they sum to; the level grid is printed only when something is open. The seven header assertions in `tests/test_pm_tools*.py` follow the new shape; `--json` is unchanged
- **devils-advocate** - the fanout stop is the adjudicator's judgment, not a ratio (DEF-ADVR-50). Every adjudication now says whether the loop is `converging` or `spiralling`, with its reason, and `FANOUT_STOP` fires on two consecutive refining rounds judged spiralling; `fanoutTraced`/`fanoutTotal` stay as evidence the adjudicator cites. A pinned confirm admits only in-delta findings, so the old ratio was total by construction whenever any finding existed, and a converging run (21 → 3 → 3 → 3 findings, MAJOR falling to MINOR) stopped as a spiral with a revert list that re-opened confirmed MAJORs. `spiralStreak` replaces `highFanoutStreak` in the loop state; loop-spec invariant 5, the adjudicator agent and the guardrail tests follow
