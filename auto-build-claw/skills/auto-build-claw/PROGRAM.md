...
planning:
  ...
  phases:
    - name: RESEARCH
    - name: HYPOTHESIS
    - name: PLAN              # resolves to PLANNING::PLAN via :: namespacing - breaks objective into N iterations
    - name: RECORD
    - name: NEXT
      skippable: true
```
...
- The `PLAN` phase resolves to `PLANNING::PLAN` in the planning workflow (multi-iteration work breakdown) and `FULL::PLAN` in the full workflow (implementation planning) via `::` namespacing
- Both use `EnterPlanMode` but have distinct prompts and exit criteria appropriate to their workflow context
