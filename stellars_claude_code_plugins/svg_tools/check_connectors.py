...
@dataclass
class CardRect:
    elem_id: str; label: str; bbox: BBox

@dataclass
class Connector:
    elem_id: str; tag: str; points: list[tuple[float, float]]

@dataclass
class TextLabel:
    content: str; cx: float; cy: float; bbox: BBox
...
def _rect_label(el) -> str:
    elem_id = el.get("id", "")
    if elem_id: return elem_id
    css_class = el.get("class", "")
    if css_class: return css_class
    return (f"rect {_safe_float(el.get('width')):.0f}x"
            f"{_safe_float(el.get('height')):.0f} "
            f"@({_safe_float(el.get('x')):.0f},{_safe_float(el.get('y')):.0f})")

def _min_bbox_to_seg_dist(bb: BBox,
                          x1: float, y1: float, x2: float, y2: float) -> float:
    """Min distance from bbox edge sample points to a line segment."""
    pts = [(bb.x, bb.y), (bb.x2, bb.y), (bb.x, bb.y2), (bb.x2, bb.y2),
           (bb.cx, bb.y), (bb.cx, bb.y2), (bb.x, bb.cy), (bb.x2, bb.cy)]
...
def parse_svg(filepath: str) -> tuple[list[CardRect], list[Connector], list[TextLabel]]:
    ...
    for el in root.iter():
        ...
        if tag == "rect" and _is_card_element(el):
            ...
            if w > 0 and h > 0:
                cards.append(CardRect(el.get("id", ""), _rect_label(el),
                                      BBox(x, y, w, h)))
        elif tag == "line":
            ...
            y_top = y - fs * 0.85
            x_left = x
            if anchor == "middle": x_left = x - est_w / 2
            elif anchor == "end": x_left = x - est_w
            labels.append(TextLabel(content, x_left + est_w / 2,
                                    y_top + fs / 2, BBox(x_left, y_top, est_w, fs)))
    ...
    for c in connectors:
        ...
        if c.tag == "line" and len(c.points) == 2:
            ...
            if abs(x2 - x1) <= tol and abs(y2 - y1) <= tol:
                issues.append(f"[zero-length] Line id={cid} at ({x1:.0f},{y1:.0f}) has zero length")
        if c.tag == "polyline":
            for i in range(len(c.points) - 1):
                ax, ay = c.points[i]
                bx, by = c.points[i + 1]
                if abs(bx - ax) <= tol and abs(by - ay) <= tol:
                    issues.append(f"[zero-length] Polyline id={cid} has duplicate point at ({ax:.0f},{ay:.0f})")
    return issues


def check_edge_snap(connectors: list[Connector], cards: list[CardRect],
                    snap_tol: float = 3.0) -> list[str]:
...
def check_label_clearance(connectors: list[Connector], labels: list[TextLabel],
                          proximity: float = 20.0, min_clearance: float = 8.0) -> list[str]:
