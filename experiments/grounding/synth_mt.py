...
UNITS = SYNTH_DIR / "units.json"
TRANSL = SYNTH_DIR / "translations.json"
VERIFY = SYNTH_DIR / "verify.json"
...
def cmd_select() -> None:
    ...
    for i, r in enumerate(pool):
        ...
        if k in seen or len(r["claim"].strip()) < 20:
            continue
        seen.add(k)
        units.append({"sid": f"s{len(units):04d}", **r})
        if len(units) >= cap:
            break
...
def cmd_verify(model: str, workers: int) -> None:
    units = json.loads(UNITS.read_text())
    trans = json.loads(TRANSL.read_text())
...
def cmd_build(translator: str, verifier: str) -> None:
    units = {u["sid"]: u for u in json.loads(UNITS.read_text())}
    trans = json.loads(TRANSL.read_text())
    ...
    for lang, by_sid in trans.items():
        ...
        for sid, translation in by_sid.items():
            ...
            rows.append({
                ...
                "origin": "synthetic_mt", "source_sid": sid, "source_corpus": u["source_corpus"],
                "source_lang": "en", "target_lang": lang,
                "translator_model": translator, "verifier_model": verifier,
                "verified": True, "verify_method": "claude_p_equivalence",
            })
    df = pd.DataFrame(rows)
    df.to_parquet(OUT)
    by_lang = df.groupby("target_lang").size().to_dict() if len(df) else {}
    print(f"wrote {len(df)} verified synthetic non-English negatives -> {OUT}")
