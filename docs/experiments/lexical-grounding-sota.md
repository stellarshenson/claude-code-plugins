...
- **Cross-lingual weights are threshold-gated, not retrained** - the Round 9 fix is a per-language decision threshold over the existing English-trained weights; it works because the MT-bridge `r1_mt` ranking transfers, but the weights themselves still learn from only 139 real non-English negatives across 16 languages. A weight-level retrain on gold v2 improves the gold-en macro-F1 but breaks English precision e2e tests and is unnecessary for the non-English fix; the durable path is more non-English negatives (Round 10 synthetic translation), which could let a single global threshold retire the language-conditional cut
