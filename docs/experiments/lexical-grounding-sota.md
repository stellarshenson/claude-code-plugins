...
- **Cross-lingual weights are threshold-gated in production, retrain validated offline** - the shipped Round 9 fix is a per-language decision threshold over the English-trained weights; it works because the MT-bridge `r1_mt` ranking transfers, but the shipped weights themselves learn from only 139 real non-English negatives. Round 10 closed the gap offline: 1,053 verified synthetic negatives let a single global threshold reach real-slice non-English TNR 0.683 with leave-one-language-out generalisation (0.60-0.79), proving the durable fix. Shipping it is gated on the English no-regression guard (recalibration breaks English precision e2e tests); until that reconciles, the threshold patch ships and the synthetic-retrain stays an offline result. de back-translation is also still uninstalled, degrading `r1_mt` for German at inference
