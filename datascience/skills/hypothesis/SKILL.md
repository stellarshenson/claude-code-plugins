...
- **On request** - whenever the user asks for a summary / recap / status of the hypotheses or experiment, print this table (pre-registration form before any run; finding form with the Verdict column once results exist) - never make them dig it out of the doc
...
Summary table to mirror (rename the claim column for what the project tests, else keep `Hypothesis under test`; Verdict is blank at pre-registration, filled by the finding):
