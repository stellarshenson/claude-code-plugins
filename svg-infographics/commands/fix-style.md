...
- **validation**: check_contrast.py, check_overlaps.py
...
2. **Run diagnostics** (create tasks per checker):
   - `check_contrast.py --svg {file} --show-all` - identify FAIL and warn entries
   - `check_overlaps.py --svg {file}` - identify spacing violations
   - `check_alignment.py --svg {file}` - identify grid snap issues
