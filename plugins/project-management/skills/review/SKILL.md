...
6. **Fix through `pm-tools`, never by hand** - lock each item before you write to it (`pm-tools lock FILE --id ID --author @xx`) and unlock when you leave it, then `amend` to reword, `edit` for any other field, `log` to record what the review found, `reject` with a reason for an item the review killed. The review is itself an event worth logging on the items it touched
