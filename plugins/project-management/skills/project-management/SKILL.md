...
Every collection of items the user is shown is a markdown table, computed by `pm-tools` and pasted verbatim - never a bulleted list, never prose, never re-typed. Four query surfaces, one filter vocabulary:
...
- **SUMMARY answers two questions, one grid each** - how much is left, and how bad. Status first: `| Category | Open | Fixed | Rejected | Total |` on defects, `Done` on criteria, a `Worked on` column after Open when an item is locked. Then the open items by level under `Open by severity` (`| Category | CRITICAL | MAJOR | MEDIUM | MINOR | Open |`) or `Open by importance` (CRITICAL, HIGH, MEDIUM, LOW) - a breakdown of the Open column alone, printed only when something is open, with an `UNTRIAGED` / `UNRATED` column only when an open item lacks a level. Each grid closes with a Total row. ITEMS is a fix queue, not an inventory - OPEN items only, grouped by category and worst level first inside each category; closed and rejected work is counted, not enumerated
