...
The reader wants what is left and what to fix first, so ITEMS lists OPEN items only, grouped by category and worst level first inside each category (severity on defects, importance on criteria), oldest id first inside a level.
...
- **`--plain`** - the SUMMARY grid and the ITEMS queue without the icons or the CATEGORIES table. The counts line, the section blurb and the tables themselves are unchanged, so plain drops the chrome and never the data
