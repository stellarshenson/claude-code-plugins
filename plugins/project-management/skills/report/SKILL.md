...
6. **Asked for a summary, run `--summary`** - it stops at the SUMMARY grid and lists no items, and it is plain by itself. Asked for something plain or short of a full report, `--plain` gives the grids and the queue with no icons and no CATEGORIES. The grids are plain counts - status per category, then the open items per level - and need no legend
