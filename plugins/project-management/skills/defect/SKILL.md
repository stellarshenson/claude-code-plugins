...
6. **Symptom first, in the reporter's terms**; `cause under investigation` is a legitimate value. The `--repro` line is written for whoever will act on it. Write every field laconic - one reading only, nothing past the mechanism, the site, the numbers and the harm, and numbers, ids, paths and error strings copied exactly (`SKILL.md`, Writing the text)
