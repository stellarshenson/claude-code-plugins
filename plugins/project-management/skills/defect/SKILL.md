...
6. **Symptom first, in the reporter's terms**; `cause under investigation` is a legitimate value. The `--repro` line is written for whoever will act on it. Write every field in plain words - one reading only, compression welcome, numbers, ids, paths and error strings copied exactly (`SKILL.md`, Writing the text)
