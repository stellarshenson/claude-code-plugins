...
5. **One assertion per item** - a criterion needing "and" is two criteria; edge cases are their own items, enumerated across the whole fanout. Write every field laconic - one reading only, nothing past the mechanism, the site, the numbers and the harm, and numbers, ids and paths copied exactly (`SKILL.md`, Writing the text)
