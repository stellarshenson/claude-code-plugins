...
5. **One assertion per item** - a criterion needing "and" is two criteria; edge cases are their own items, enumerated across the whole fanout. Write every field in plain words - one reading only, compression welcome, numbers, ids and paths copied exactly (`SKILL.md`, Writing the text)
