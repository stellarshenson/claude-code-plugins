...
8. **Closing needs proof** - `close` refuses without `--evidence`: one line saying what shows the defect is gone, the regression test that passes or the build the repro no longer fires on. Do not close on the strength of having written a fix; run something first and record what it said
9. **A fixed defect that breaks again is a regression, not a reopen** - `reopen DEF-LNCH-3` files `DEF-LNCH-3-1` and leaves the original closed with its proof intact. Record how the regression reproduces with `edit --repro`; it need not be the old one. Never hand-edit a closed defect back to open - the ordinals are what make regressions countable
