...
The `svg-infographics:svg-infographics` skill (fork context) reads `references/validation.md` for checker usage, severity ladder (HARD FAIL / SOFT / HINT), justification rules, pre-delivery checklist. For heavy validation work across many files, invoke it via the `Skill` tool: `Skill(skill="svg-infographics:svg-infographics", args="Validate <paths>. Run finalize, batch-fix, classify findings.")` to keep main session responsive, or one `svg-infographics:svg-builder` agent per batch of files when each batch should be visible and stoppable on its own.
