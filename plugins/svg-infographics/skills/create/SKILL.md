...
4. **Dispatch the builder** per the execution plane chosen in step 1.
   ...
   **4a. In-session fork** - ONE `Skill` call per deck; sibling images for one document are ONE deck, capped at 5. Above 5, split into multiple decks and make one call each - one call per image would waste ~100KB context per fork. The fork runs on the plugin's `svg-infographics:svg-builder` agent - eight tools, no MCP schemas - so the context it costs is the skill and the deck, not a general-purpose tool surface.
   `Skill(skill="svg-infographics:svg-infographics", args="Create <deck description, all images> into the absolute directory <output-dir> - write files there verbatim, do NOT nest or re-derive the path - from the approved concept draft: <draft>. Follow the workflow per image, sequentially. Theme <swatch>. Close with the deck consistency check.")`
   ...
   **4b. Background agents** - ONE `Agent` per graphic, all dispatched in a single message so they run concurrently. Name each agent for the graphic it owns so the agent list is readable.
   The agent type is the plugin's `svg-infographics:svg-builder`: it reads the umbrella skill itself and carries the eight-tool grant and the sonnet pin in its own definition, so pass neither a skill path nor a model. An `Agent` prompt is literal text - no shell runs on it - so the output path in it must already be absolute.
   `Agent(subagent_type="svg-infographics:svg-builder", description="<NN> <graphic name>", prompt="Build EXACTLY ONE graphic: <this image's spec from the draft>. Write it to the absolute path <output-dir>/<NN-name>.svg - that exact path, do NOT nest or re-derive it. Theme <swatch>. Run the full per-image workflow including preflight, check and finalize.")`
...
5. **Builder workflow** (per image, both planes):
   ...
   - **Content** - text (CSS classes only), icons (Lucide ISC, corner slots via `place --ref-id accent-bar`), descriptions. A real object (a chair, an anvil, a building) is a wireframe fragment: `mesh search` then `wireframe --model <slug>` (`rules/shapes.md`). Unicode glyphs only, no ASCII arrows.
