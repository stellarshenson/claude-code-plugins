...
- **Verdict** - one of Ships / Kept / Promoted / Confirmed / Blocked / Dropped / Refuted / Refuted (null) / Killed-at-gate, with the number that justifies it; the label opens the bullet and the justification follows a `;`, so `hypothesis-tools` can read it - a label outside this set is a `check` error. This list is the single home for the vocabulary; SKILL.md and `ledger-queries.md` point here rather than restating it, because the two copies had already drifted apart
...
- **Declaration line** - the id comes FIRST, either in a plain heading (`### E30-H106 turbomind-throughput`) or opening a compact bullet's bold label (`- **E30-H106 turbomind-throughput** - prose`); a bold id in a heading is fine. Anything else - the id mid-label, a setext heading - is invisible to `hypothesis-tools`, which reports it as an error rather than dropping it
