...
- **`hypothesis-tools attach <log> <id> --path P [--path P]... --author @xx`** - write `- attachment: <path> sha256:<16 hex> edited:<ISO 8601 UTC stamp>` per artefact after the lock line and before the fields, the path relative to the ledger's directory so ledger and artefacts move together; a path already attached is refreshed in place and the log line carries `sha256:<old> -> sha256:<new>`, an unchanged one writes nothing; a missing file is refused before anything is written. `check` recomputes the fingerprint: a changed or missing artefact is a warning, a malformed line or a path attached twice is an error
