...
**Multi-round reviews run as a deterministic workflow** where the session has the dynamic Workflow tool: `skills/adversarial-review/workflows/adversarial-loop.js` encodes the whole loop as code - forced adjudication before any fix, verdicts computed from the severity mix (any CRITICAL or MAJOR blocks) rather than trusted from reviewer prose, confirming rounds pinned to the closure list and fix delta, and hard exits (`STOP`, `FANOUT_STOP`, `ROUND_CAP`) when the loop starts generating its own work. The orchestrator authors only the arguments - target, scope, the mandatory product `bar`, lenses - never the control flow, so the protocol cannot drift with a long session's context.
