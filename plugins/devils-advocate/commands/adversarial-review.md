...
6. **Multi-round with the dynamic Workflow capability present - construct the workflow from the spec, do not drive rounds by hand**: the spec the plugin owns and ships (`${CLAUDE_PLUGIN_ROOT}/skills/adversarial-review/references/loop-spec.md`) is the binding contract - construct the workflow according to it (the worked example at `${CLAUDE_PLUGIN_ROOT}/skills/adversarial-review/workflows/adversarial-loop.js` is consultable starting material), check it against the spec's invariant list, and run it. This command is the Workflow opt-in. The `bar` argument is mandatory and is an OBJECT: `purpose` (what the product is for, and for whom), `inputs` (the input universe it handles - what a user actually feeds it), `primaryPath` (the use every CRITICAL/MAJOR must sit on), plus optional `guarantees`, `outOfScope`, `degrade`; write it from the README and the user's words, and confirm it with the user in one line before the first spawn - an output-only bar ("never fuse two values") is what turned a `<select>` nobody pastes into a MAJOR, a normalisation pass and 1.4M tokens. Pass `graph` when a refreshed graphify graph exists, and ASK the user whether to build one when it does not. On a `PLAN` return, apply in the main session first `reverts` (remove each listed mechanism), then exactly `plan` - read `mechanisms` before applying and veto any that does not answer a material CRITICAL/MAJOR - run the tests, and re-invoke with the returned `state` plus `appliedFixes` (`files` listed per fix); relay `STOP`/`FANOUT_STOP`/`ROUND_CAP` to the user - never a licence to keep looping by hand. Steps 7-11 below are the supplied fallback for harnesses without the dynamic Workflow capability - follow the worked example stage by stage as the checklist
