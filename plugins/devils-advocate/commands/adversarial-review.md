...
6. **Multi-round with the Workflow tool present - run the canonical workflow, do not drive rounds by hand**: `Workflow({scriptPath: "${CLAUDE_PLUGIN_ROOT}/skills/adversarial-review/workflows/adversarial-loop.js", args: {target, scope, bar, lenses, testCmd}})`. This command is the Workflow opt-in. The `bar` argument is mandatory - what the product must do and which inputs are out of scope; the script refuses to run without it. Relay the terminal status and its history to the user; a non-SHIP status (`STOP`, `FANOUT_STOP`, `ROUND_CAP`, `FIX_FAILED`) is a user decision point, never a licence to keep looping by hand. Steps 7-11 below are the manual fallback for sessions without the Workflow tool
