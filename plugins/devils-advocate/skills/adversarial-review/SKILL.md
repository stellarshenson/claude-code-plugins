...
- **Construct, check, run** - design the loop with the harness's own workflow practices (stage graph, verifiers, budgets, names are yours), then check the script against the spec's invariant list one by one before running it. Worked example: `${CLAUDE_PLUGIN_ROOT}/skills/adversarial-review/workflows/adversarial-loop.js` - consult it for the invariant mechanics, not the loop shape
