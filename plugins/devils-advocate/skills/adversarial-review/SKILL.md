...
- **Measure each round from its own run directory** - the launch result names it; `review-tools cost <that directory> --by lens` gives the round's elapsed wall clock and its tokens per adversary and per stage, since the script labels every spawn `<stage>:<lens>`. Report it with the round's findings and keep the directories: the review's total is the same call over all of them. A workflow script cannot time itself - the clock and the filesystem are both closed to it - so the ledger is read after the invocation returns, never built inside it
