...
- **On `PLAN`** - apply in the main session first `reverts`, then the plan - exact changes, smallest radius, nothing else; read `mechanisms` first and veto any that does not answer a material CRITICAL/MAJOR; write the diff of each change to a file as you apply it; run the tests; re-invoke with `args.state` from the return plus `args.appliedFixes` carrying `files` and `patch` (that diff file's path) per fix, which the script requires. Run the script from its installed path on every invocation, never a copy kept from an earlier session: every return's `state.contract` must equal `LOOP_CONTRACT` in the installed file, and a return without one came from a stale copy. Changes land only between invocations, visible and interruptible
- **Measure each round from its own run directory** - the launch result names it; a workflow script cannot time itself - the clock and the filesystem are both closed to it - so the ledger is read after the invocation returns, never built inside it, and reported with the round's findings (the call is in *After the review*)
...
- **Code graph - offer once at launch** - a refreshed index passed as `args.graph` cuts reviewer turns (one index lookup replaces a dozen greps - turns are the token bill) and lets the adjudicator group findings by cause and bound each change's radius, which is what keeps plans small and fixes clean. Queries, costs, traps: `references/code-graph-instrument.md`. `tmp/graphify-out/graph.json` exists → `graphify update` (AST-only, free), use, no question. No index → one `AskUserQuestion` before the discovery round: AST-only (default) / LLM-assisted (`graphify extract`, billed) / none; `graphify` absent → the same question also asks to install it (`uv tool install graphifyy`). Build the pick. Refresh AST-only after each fix round, no question. LLM pass only when picked
- **Research - opt-in, budgeted, cached** - scope is best practices, paradigms and patterns, nothing else. A reviewer asks in a finding's `research` field, the adjudicator approves or not, and every return carries `research.queue` and `research.suggestions`. For each queued question run `review-tools research search` over the lens's `references/<lens>/research.md` and the project cache `.claude/review-research/<lens>/research.md`; an entry that answers it means no research. A miss with no permission yet → ask the user once: No (default) / 1 / 3 / 5 research units per adversary. No → `args.research = {allowed: false}`, never ask again this review. A number → `{allowed: true, budget: N, used: {}}`; research each miss per `references/research-brief.md` while `used[lens] < budget` (one unit = one question to completion, as many passes as it needs), append it to the project cache, count it in `used`; misses past the budget join the suggestions. The skill never asks for more budget: put each suggestion to the user with No as the default; yes researches that one question into the cache, budget unchanged. Research runs between or after invocations, never inside one; pass `args.research` on every re-invocation
...
A hostile reviewer on freshly written remedy code near-never returns clean, so a Stop hook or goal that opens only on a clean verdict is an unbounded loop. The two terminal states: a clean round (`SHIP`), or an adjudicator `STOP`/`FANOUT_STOP` put to the user; a gate accepts both.
...
| `ux-designer` | friction & intent, hierarchy, cognitive load, static text & tooltips, design-language reuse, motion comfort, accessibility, edge states | 2 |
...
| `digital-marketer` | marketing copy - message, concreteness, calls to action, claim substantiation, channel mechanics, AI-sounding phrasing | 2 |
...
- **Label every spawn `<stage>:<lens>`** - `discover:bug-hunter`, `confirm:architect`. The harness records the label beside the transcript, and it is the only thing that says which adversary spent which minutes and tokens; without it a finished review is a heap of agent ids
...
- **Measure the review** - `review-tools cost <the round's run directory, or its transcripts> --by stage|lens|run` (a workflow's own `workflows/wf_*` directory is one round; a hand-spawned round is its `agent-*.jsonl` files in the session's subagent transcript directory). It reports elapsed wall clock against the agent minutes inside it, tokens, tool mix and re-reads per stage and per adversary; cache read is the bill, and a prompt change that did not cut turns saved nothing. A `claude -p` reviewer run with `--no-session-persistence` leaves no transcript, so it cannot appear in the ledger - say so rather than reporting the panel short
