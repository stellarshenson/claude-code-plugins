...
5. The loop exits only on the required number of consecutive clean confirming rounds, an adjudicator `STOP`, a fanout stop, or a round cap. The fanout stop is the adjudicator's judgment, never a ratio: every adjudication says whether the loop is converging or spiralling, with its reason, and the stop fires on two consecutive rounds judged spiralling whose adjudication still ordered changes or reverts. A clean round - no findings, or adjudicated clean - is clean whatever its findings' lineage and resets the streak. The share of findings sitting in the loop's own fixes is evidence the adjudicator cites, not the gate - a pinned confirm admits only in-delta findings, so that share is total by construction whenever any finding exists, and a converging round with one MINOR left would otherwise stop as a spiral (DEF-ADVR-50). [`cleanRequired`, `FANOUT_STOP`, `ROUND_CAP`, `maxRounds`, `spiralStreak`, `trajectory`]
