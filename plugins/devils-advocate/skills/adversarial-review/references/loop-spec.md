...
Its args contract (target, bar, lenses mandatory; the script throws without them, on a bar missing purpose, inputs or primaryPath, and on a re-invocation whose `state` was written under another `LOOP_CONTRACT` or whose applied fixes lack `files` or `patch`):
