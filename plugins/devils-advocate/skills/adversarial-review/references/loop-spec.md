...
4. Confirming rounds are pinned: the closure list plus the applied delta is the whole attack surface, never a fresh sweep - and the script enforces it: a confirming finding that is taste, or sits outside the applied delta without naming a closure it fails or whose change caused it, is discarded before adjudication, logged by title, never silently. A confirming reviewer reads the diff of exactly the applied change, never the whole delta again, and a finding at a site already ruled, which no fix has touched since the ruling, is discarded the same way. [`pinFilter` on `closure` / delta-file membership; `appliedFixes[].patch` in the confirm prompt; `settledFilter` over the `settled` record]
5. The loop exits only on a clean round, an adjudicator `STOP`, a fanout stop, or a round cap. A clean round ships: nothing edits the tree inside the workflow, so a second panel would review identical code (DEF-ADVR-65). The fanout stop is the adjudicator's judgment, never a ratio: every adjudication says whether the loop is converging or spiralling, with its reason, and the stop fires on two consecutive rounds judged spiralling whose adjudication still ordered changes or reverts. A clean round - no findings, or adjudicated clean - is clean whatever its findings' lineage. The share of findings sitting in the loop's own fixes is evidence the adjudicator cites, not the gate - a pinned confirm admits only in-delta findings, so that share is total by construction whenever any finding exists, and a converging round with one MINOR left would otherwise stop as a spiral (DEF-ADVR-50). [`SHIP` on the first clean round, `FANOUT_STOP`, `ROUND_CAP` checked before a re-invocation's panel, `maxRounds`, `spiralStreak`, `trajectory`]
...
- **The workflow's own name** - a constructed loop names itself for the target it is reviewing, not for the protocol it implements: `review-turndown-diff`, `audit-clipboard-path`, `confirm-lock-delta`. Follow the harness convention - `meta.name` kebab-case and lowercase, `meta.description` a one-line lowercase phrase, both PURE LITERALS because the harness parses `meta` before the script runs, so neither can interpolate. The worked example's `adversarial-review-loop` is its own name, not a name to inherit
- **Names and shapes** - argument names, status names, schema field names and return shapes are the worked example's conventions, not the contract; a constructed loop keeps the concepts (bar, closure list, applied delta, plan, reverts, prior record, history, research requests and their routing to the main session) and may name them as it likes, as long as the main session can read the payload it receives. The one name that is not free is each spawn's label - `<stage>:<lens>`, and `<stage>:r<round>` for a per-round agent: the harness records the label beside the transcript, so it is what lets the finished run be measured per stage and per adversary instead of per opaque agent id
...
- **Instruments are data, never prescriptions** - pass an agent what exists and where, and let it decide whether and how to use it. Never spell out a command: an invocation written into a prompt pins an API that moves, and a hardcoded approach overfits to the problem class it was written for while misclassifying the rest. The agent reads the tool's own help for the surface it has today, and says so in a line when it falls back

What stays fixed is only the list above: a bar with purpose, inputs and primary path; adjudication before change; materiality before severity; revert before refine; pinned confirms filtered by the script; the exit set; no edits from inside the workflow; a plan-exit round-trip through the main session; full history and the research routing on every return.
...
Its args contract (target, bar, lenses mandatory; the script throws without them, on a bar missing purpose, inputs or primaryPath, and on a re-invocation whose `state` was written under another `LOOP_CONTRACT` or whose applied fixes lack `files` or `patch`, and on a `research` permission outside its shape):
...
| `graph` | optional path to a refreshed code-graph index - fewer reviewer turns, cause-grouping and radius-bounding for the adjudicator. Passed as data (what it is, where); the agent decides whether and how to use it |
| `state` | re-invocation only: the `state` object from the previous `PLAN` return, verbatim; it carries the `contract` it was written under |
| `appliedFixes` | re-invocation only, required: `[{site, summary, files, patch}]` the main session actually applied, reverts included, each revert recorded with summary starting `reverted: <mechanism>`; `files` the paths it touched, `patch` the path of a file holding the diff of exactly that change - confirming reviewers read it instead of the whole delta |
...
| `research` | optional: the user's research permission, asked once per review by the main session and carried in `state` - `{allowed: false}`, or `{allowed: true, budget: 1, 3 or 5, used: {<lens>: n}}`; absent means not asked. A reviewer requests best-practice, paradigm or pattern research on a finding, the adjudicator approves, and every return carries `research.queue` (check against the cache, research between invocations) and `research.suggestions` (approved for a lens whose budget is used up) |

Its statuses: `PLAN` (the adjudicated change plan for the main session to apply - `reverts` first, then `plan`; `mechanisms` lists the changes that add review surface, for veto; carries `state` for the re-invocation), `SHIP`, `STOP` (adjudicator ruling), `FANOUT_STOP`, `ROUND_CAP`, `ADJUDICATOR_DIED`. `STOP` and `FANOUT_STOP` carry `reverts` - the adjudicator's list (`{mechanism, site, dissolves, defers}`), else every applied change in closure shape (`{site, summary, files}`), entries whose summary starts `reverted:` being reverts already applied and skipped - so the main session reverts deterministically. Every status returns the full round history, findings, closures, deferrals, refutations and the research routing.
