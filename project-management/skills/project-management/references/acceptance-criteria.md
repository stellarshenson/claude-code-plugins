...
- [x] `ACC-BRSW-1` **Submenu** - row with >1 conversation JSONL shows "Switch Conversation Branch"
  - test: seed a project with 2 JSONLs, right-click the row
  - test-tags: unit, e2e
  - log: 2026-06-12T08:41:03Z @kj implemented (v1.2.2)
- [ ] `ACC-BRSW-2` **Names** - submenu entries show conversation names, never the shared project path
...
- [-] `ACC-BRSW-3` **Edge: branch removed before click** - switch returns 404, panel shows the error and refreshes
...
Every criterion carries two more sub-lines. Both are one line, and both feed the report.
...
- **`- test-tags:`** - which kinds of test cover it: `unit`, `integration`, `functional`, `e2e`, `manual`. Reuse the words across the file - `report` counts them, and that count is the answer to "how many criteria are covered by unit tests"
- An untested criterion is tagged `manual` or left untagged, never tagged for a test that does not exist. The coverage table is only worth reading while it is true
...
- [ ] `ACC-BRSW-8` **Read-only: no submenu** - a read-only link never renders "Switch Conversation Branch"
  - log: 2026-06-12T13:38:19Z @kj added
- [ ] `ACC-BRSW-9` **Viewer: switch without persist** - a viewer switch updates the panel and leaves the JSONL mtime untouched
...
pm-tools add docs/acc-crit-app.md --category AUTH --name Authentication --author @kj \
    --description "Login, session lifetime and password handling" \
