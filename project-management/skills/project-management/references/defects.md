...
Two more sub-lines, one line each. The `repro:` line is the one another person acts on, so it is written for them, not for the record.
...
pm-tools close  docs/defects-app.md --id DEF-LNCH-3 --author @kj \
    --event "fixed: token awaited before the first turn; 79 pytest green"
